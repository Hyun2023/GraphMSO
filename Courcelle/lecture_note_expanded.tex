...
\[
  (G_t,R_t,\iota_t) \defeq (G[\cone(t)],\adh(t),c|_{\adh(t)}),
\]
where only the adhesion is kept as the external root for the cone.
...
\(G[\cone(t)]\).  Thus the glued graph is exactly \(G_t\), with root \(R_t\) and
external labels inherited from \(H_t\), as claimed.
\end{proof}
...
\begin{definition}
A \(\Sigma_\omega\)-tree \((T,\rho)\) is \emph{legal} if for every node \(x\) with
parent \(y\), writing \(\rho(x)=(H_x,R_x)\) and \(\rho(y)=(H_y,R_y)\), we have
\[
  R_x\subseteq V(H_y)
  \quad\text{and}\quad
  H_x[R_x]=H_y[R_x].
...
node carries exactly one letter, the finite disjunction says exactly that the
...
so the same argument glues the second child.  Finally, by definition of the cone
root at \(t\), all labels outside \(R_t\) are forgotten.  Therefore the remaining
labelling is defined exactly on \(\bar R_t\).
\end{proof}
...
\begin{proof}
By Lemma~7, the graph at the root is well-defined.  Since the root has no parent,
its external root is empty, so it is an ordinary graph.  The color of a decoded
