...
Fix the encoding \((T,\rho)\) of \((G,c,(T,\beta))\); by
Lemma~\ref{lem:legal-iff-enc} its decoded graph is \(G\), and every vertex \(v\)
and vertex set \(S\) has the defining tuple \(\tuple U^v\), \(\tuple U^S\)
introduced above.  Given a \(\tau_{\Pred}\)-MSO{} formula \(\theta\) and a
...
defining tuple, so the atomic equivalences proved above apply directly:
Lemma~\ref{lem:eq} for \(x=y\), Lemma~\ref{lem:adj} for \(\adj(x,y)\),
Lemma~\ref{lem:pred} for \(Q(x)\), and Lemma~\ref{lem:cont} for \(x\in X\).  Each
...
\(\varphi_{\vtx}\) by Lemma~\ref{lem:vtx}, and the induction hypothesis for
...
\(\varphi_{\vtx}(W)\); since \((T,\rho)\) is legal, Lemma~\ref{lem:vtx} gives
\(W=\tuple U^a\) for some vertex \(a\).  The induction hypothesis for \(\psi\) and
...
\(\varphi_{\set}\) and Lemma~\ref{lem:set}: every vertex set has a defining tuple
satisfying \(\varphi_{\set}\), and every tuple satisfying \(\varphi_{\set}\) is the
defining tuple of a vertex set.
