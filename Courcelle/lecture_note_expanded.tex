...
\begin{theorem}[Courcelle's theorem for unary-expanded graphs]
...
\(G[\cone(t)]\).  The same union argument applies to each unary predicate,
because each piece carries the restriction of the predicate interpretation from
the same ambient structure \(G\).  The root of the result is \(R_t=\adh(t)\) and,
since gluing inherits the labelling of its first operand, its labels are
\(c|_{\beta(t)}\), matching the cone graph at \(t\).  Thus the glued
...
\(\tau_{\Pred}\)-graph \((H_t,R_t,\iota_t)\) by replacing each color
\(i\in V(H)\) by a formal vertex \(v(t,i)\), transporting the edge relation and
all unary predicates along this renaming, taking
\(R_t=\{v(t,i):i\in R\}\), and setting \(\iota_t(v(t,i))=i\).

Define rooted \(\tau_{\Pred}\)-graphs
\((\bar G_t,\bar R_t,\bar\iota_t)\) bottom-up.  At a leaf,
...
\(\bar R_{t_i}\).  Legality says that the rooted \(\tau_{\Pred}\)-graph induced
by this labelled root agrees with the substructure of the parent letter on the
same labels.  Hence
...
root is an ordinary \(\tau_{\Pred}\)-graph, with no remaining external labels.
The color of a decoded vertex is well-defined because vertices are identified
only when their labels agree; thus two representatives of the same decoded
vertex have the same color.  The unary predicates are also well-defined because
gluing is allowed only when all predicate tags agree on the identified labelled
root vertices.

It remains to check that \((T,\beta)\) is a tree-decomposition of the underlying
graph of \(G\).  Every vertex of \(G\) is created as some \(v(t,i)\), hence
appears in \(\beta(t)\).  No edge is created by gluing; every edge of the
underlying graph of \(G\) comes from a node graph \(H_t\), so both endpoints
occur in \(\beta(t)\).  For connectedness, fix a decoded vertex
...
\(R_t\), the edge relation is the one recorded in \(H_t\), and each unary
predicate \(P\in\Pred\) holds exactly on the colors tagged by \(P\) in \(H_t\).
Thus re-encoding the decoded triple returns \((T,\rho)\).
\end{proof}
