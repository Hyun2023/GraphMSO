...
All graph-specific omitted proofs used in the encoding and translation argument
are expanded here.  The standard binary/nice tree-decomposition normalization
theorem is stated explicitly below; its detailed construction is kept in the
companion file \texttt{nice\_tree\_decomp.tex}.
\end{abstract}
...
\begin{theorem}[Binary/nice normalization]\label{thm:binary-nice-normalization}
Let \(G\) be a finite graph and let \((T,\beta)\) be a finite
tree-decomposition of \(G\) of width at most \(\omega\), with \(m=|N(T)|\).  There
is an algorithm which, for fixed \(\omega\), runs in time \(O_\omega(m)\) and
outputs a rooted ordered nice tree-decomposition \((T',\beta')\) of \(G\) of
width at most \(\omega\).  In particular \(T'\) is binary, the root and leaves
may be required to have empty bags, every node is a leaf, introduce, forget, or
join node, and
\[
  |N(T')|\le C_\omega m
\]
...
\begin{proof}
This is the standard nice-decomposition normalization theorem.  In this project
the detailed proof is separated into \texttt{nice\_tree\_decomp.tex}: root the
input decomposition, replace each tree edge by a path that changes one bag
element at a time, make leaves and the root empty, and replace every high-degree
branching point by a binary join tree.  Each inserted path has length bounded by
a function of \(\omega\), and each branching replacement has size linear in the
number of old incident child subtrees.  Thus the total number of new nodes, and
the time needed to write them down, is \(O_\omega(m)\), while no bag ever has
size larger than \(\omega+1\).
\end{proof}
...
bounded by a function of \(\omega\).  The decomposition written down by the
linear-time approximation algorithm has \(O_\omega(|V(G)|)\) nodes.  Apply
Theorem~\ref{thm:binary-nice-normalization} to convert it to a binary, indeed
nice, decomposition with only a linear blow-up.  Then apply
Theorem~\ref{thm:courcelle} with the new width bound.  Since \(\omega\) is a
parameter, changing the width bound only changes the computable parameter
function \(f\).
\end{proof}

\begin{lemma}\label{lem:tw-edges}
If a simple graph \(G\) admits a tree-decomposition of width at most \(\omega\),
then \(|E(G)|\le\omega\,|V(G)|\).
\end{lemma}

\begin{proof}
First, if \(V(G)\ne\varnothing\) then \(G\) has a vertex of degree at most
\(\omega\).  Choose a tree-decomposition \((T,\beta)\) of width at most \(\omega\)
with the fewest nodes, and root \(T\).  If \(T\) is a single node \(r\), then
\(V(G)\subseteq\beta(r)\), so \(|V(G)|\le\omega+1\) and every vertex has degree at
most \(\omega\).  Otherwise let \(t\) be a leaf with parent \(s\).  By minimality
\(\beta(t)\not\subseteq\beta(s)\), since otherwise deleting \(t\) would give a
smaller decomposition; fix \(v\in\beta(t)\setminus\beta(s)\).  The nodes whose bag
contains \(v\) form a connected subtree containing the leaf \(t\), and its only
neighbour \(s\) omits \(v\), so that subtree is \(\{t\}\).  Every neighbour of
\(v\) lies in a common bag with \(v\), necessarily \(\beta(t)\); hence
\(\deg(v)\le|\beta(t)|-1\le\omega\).

We conclude by induction on \(|V(G)|\) that \(|E(G)|\le\omega|V(G)|\).  The case
\(V(G)=\varnothing\) is clear.  Otherwise pick \(v\) of degree at most \(\omega\)
as above; the induced subgraph \(G-v\) inherits the tree-decomposition obtained by
deleting \(v\) from every bag, again of width at most \(\omega\), so by induction
\(|E(G-v)|\le\omega(|V(G)|-1)\).  Hence
\(|E(G)|=\deg(v)+|E(G-v)|\le\omega+\omega(|V(G)|-1)=\omega|V(G)|\).
\end{proof}
...
This leaf-attachment construction has one extra node per edge and need not
preserve binary branching.  Whenever the binary input required by
Theorem~\ref{thm:courcelle} is needed, apply
Theorem~\ref{thm:binary-nice-normalization} to this incidence decomposition.
Unary predicates do not affect treewidth.  Also, by Lemma~\ref{lem:tw-edges}, a
simple graph of treewidth at most \(\omega\) has at most \(\omega|V(G)|\) edges.
Therefore \(|V(\widehat I(G))|=|V(G)|+|E(G)|=O_\omega(|V(G)|)\), and the normalized
incidence decomposition has \(O_\omega(|V(G)|)\) nodes.
...
For \(k\in\mathbb N\), write \([k]=\{1,\ldots,k\}\), with \([0]=\varnothing\), and
set
\[
  C_k\defeq\{0\}\cup[k].
\]
Thus \(C_\omega=\{0,\ldots,\omega\}\) has exactly \(\omega+1\) colours.

\begin{fact}[Bag-injective coloring]
There is a coloring \(c:V(G)\to C_\omega\) such that no bag contains two vertices
of the same color.
\end{fact}
...
\(\iota\) is a partial injective map from \(V(H)\) to \(C_k\) whose domain
contains \(R\).  When \(\iota(v)=i\), we say that \(v\) has label \(i\).
\end{definition}

An isomorphism of \(k\)-rooted \(\tau_{\Pred}\)-graphs
\[
  f:(H,R,\iota)\cong (H',R',\iota')
\]
is a \(\tau_{\Pred}\)-graph isomorphism \(f:H\to H'\) such that
\[
  f(R)=R',\qquad f(\operatorname{dom}\iota)=\operatorname{dom}\iota',
  \qquad \iota'(f(v))=\iota(v)
\]
for every labelled vertex \(v\in\operatorname{dom}\iota\).  Thus roots and all
temporary labels, not only the labels on roots, are preserved.
...
When \(k\) is clear from context we write \(\oplus\) for \(\oplus_k\).  Since
\(\oplus\) is defined by a disjoint union followed by quotient identifications,
statements involving glued graphs are stated up to rooted label-preserving
isomorphism unless a particular quotient model has been fixed.
...
\begin{enumerate}[label=\textup{(\roman*)}]
  ...
  \item Every edge of \(G[\cone(t)]\) has both endpoints in a common bag
    \(\beta(s)\) with \(s\) in the subtree rooted at \(t\).  When \(t\) has children
    \(t_1,\ldots,t_d\), it is therefore an edge of \(G[\beta(t)]\) or of some
    \(G[\cone(t_k)]\).
\end{enumerate}
\end{lemma}
...
\(t\) has children \(t_1,\ldots,t_d\), the node \(s\) is either \(t\) or lies in
the subtree rooted at some child \(t_k\); correspondingly
\(\beta(s)\subseteq\beta(t)\) or \(\beta(s)\subseteq\cone(t_k)\), and the edge
belongs to the induced subgraph on that vertex set.
\end{proof}

\begin{lemma}[Cone gluing]\label{lem:cone-gluing}
Let \(t\) have children \(t_1,\ldots,t_d\) (so \(d\le 2\) in a binary
decomposition, and \(d=0\) at a leaf).  If \((G_k,R_k,\iota_k)\) is the cone graph
of \(t_k\), then
\[
  (G_t,R_t,\iota_t)
  \cong
  (H_t,R_t,\iota_t)\oplus (G_1,R_1,\iota_1)\oplus\cdots\oplus (G_d,R_d,\iota_d),
\]
the empty gluing at a leaf giving \((G_t,R_t,\iota_t)\cong(H_t,R_t,\iota_t)\).
\end{lemma}

\begin{proof}
For each child \(t_k\), the root \(R_k=\adh(t_k)=\beta(t_k)\cap\beta(t)\) is a
subset of \(\beta(t)\).  Because the coloring is injective on \(\beta(t)\), every
label appearing in \(R_k\) identifies a unique vertex of \(H_t\).  The induced
substructure on \(R_k\) inside the child cone is the same as the induced
...
relation and all unary predicates agree on the common labelled root, so each
gluing \(\oplus(G_k,R_k,\iota_k)\) is defined.  Moreover,
Lemma~\ref{lem:cone-struct}(ii) says that two distinct child cones can overlap
only inside their adhesions, hence inside \(\beta(t)\).  Therefore, after gluing
one child cone to \(H_t\), the induced labelled structure on \(\beta(t)\) is still
the original \(H_t\); in particular the next child adhesion still matches the
corresponding induced substructure of \(H_t\).  Thus the iterated gluing displayed
above is well-defined step by step.

The vertex set of the result is
\(\beta(t)\cup\cone(t_1)\cup\cdots\cup\cone(t_d)=\cone(t)\).  The gluing identifies
a vertex of a child cone with a vertex of \(H_t\) only when they share a label,
that is, along \(R_k=\adh(t_k)\).  By Lemma~\ref{lem:cone-struct}(i),
\(\cone(t_k)\cap\beta(t)=\adh(t_k)\), so these are exactly the vertices that
\(\cone(t_k)\) and \(\beta(t)\) have in common; and by
Lemma~\ref{lem:cone-struct}(ii), for distinct children
\(\cone(t_k)\cap\cone(t_l)\subseteq\adh(t_k)\cap\adh(t_l)\), so a vertex lying in
two child cones is, through its label, identified with the same vertex of
\(H_t\).  Hence the quotient identifies precisely the shared vertices of the
pieces and nothing more.

Gluing creates no new edges; it takes the union of the edges already present in
the pieces.  By Lemma~\ref{lem:cone-struct}(iii), every edge of \(G[\cone(t)]\) is
an edge of \(G[\beta(t)]\) or of some \(G[\cone(t_k)]\), hence of one of the
pieces; conversely every edge of a piece is an induced edge of \(G[\cone(t)]\).
The same union argument applies to each unary predicate, since each piece carries
the restriction of the predicate interpretation from the same ambient structure
\(G\).  The root of the result is \(R_t=\adh(t)\) and, since gluing inherits the
labelling of its first operand, its labels are \(c|_{\beta(t)}\), matching the
cone graph at \(t\).  Therefore the canonical map from the quotient produced by
the gluing to the induced cone graph \(G_t\) is a rooted label-preserving
\(\tau_{\Pred}\)-isomorphism.
\end{proof}

\subsection{The alphabet and legal trees}

\begin{definition}\label{def:sigma-tree}
For a finite alphabet \(\Sigma\), a \emph{\(\Sigma\)-tree} is a finite rooted
ordered tree---each node having an optional first child and an optional second
child, recorded by the relations \(\child_1,\child_2\)---together with a labelling
\(\rho:N(T)\to\Sigma\).  Thus every node has at most two children, in order; a
\emph{binary tree-decomposition} is one whose underlying tree is of this form.
When an unordered rooted binary decomposition is represented in this vocabulary,
a node with exactly one child is represented by putting that child in the
first-child slot and leaving the second-child slot absent.
All tree \MSO{} formulas over the vocabulary
\(\tau_\Sigma=\{\child_1,\child_2\}\cup\{P_a:a\in\Sigma\}\), where \(P_a(x)\) holds
iff \(\rho(x)=a\), and all tree automata below are interpreted over the class of
\(\Sigma\)-trees.  Restricted to this class, the uniqueness of the parent and of
the root, acyclicity, and the presence of exactly one label per node hold by
definition and need not be axiomatised.
\end{definition}
...
representative.  Since the root letter has \(R_r=\varnothing\), no labels survive
at the root and \(G\) is an ordinary \(\tau_{\Pred}\)-graph.

\begin{remark}
The quotient definition above is the formal decoding used in the sequel.  It can
be viewed operationally as bottom-up gluing of the letters by the same operation
\(\oplus\) used in Lemma~\ref{lem:cone-gluing}, but this viewpoint is not used as
a separate lemma.
\end{remark}
...
Lemma~\ref{lem:legal-iff-enc} shows that re-encoding is a left inverse of
decoding on legal trees.  The next lemma is the companion statement in the
opposite direction: decoding the encoding of a triple returns that triple, up
to a canonical isomorphism.  It is this lemma that allows statements made
about the decoding of a legal tree to be transferred to the graph one started
from.

\begin{lemma}[Decoding inverts encoding]\label{lem:decode-enc}
Let \((T,\beta)\) be a binary tree-decomposition of a \(\tau_{\Pred}\)-graph
\(G\) of width at most \(\omega\), let \(c:V(G)\to C_\omega\) be injective on
every bag, and let \((T,\rho)\) be the encoding of \((G,c,(T,\beta))\); it is
legal by Lemma~\ref{lem:legal-iff-enc}, and we write
\((\bar G,\bar c,(T,\bar\beta))\) for its decoding.  For a slot \(v(t,i)\), let
\(\phi(t,i)\) denote the unique vertex of color \(i\) in \(\beta(t)\).  Then
\[
  \Phi\bigl([v(t,i)]\bigr)\defeq\phi(t,i)
\]
is a well-defined map \(V(\bar G)\to V(G)\), and it is an isomorphism of
\(\tau_{\Pred}\)-graphs \(\bar G\cong G\) such that
\begin{enumerate}[label=\textup{(\roman*)}]
  \item \(c(\Phi(w))=\bar c(w)\) for every \(w\in V(\bar G)\), and
  \item for every node \(t\) and every \(w\in V(\bar G)\),
    \(\Phi(w)\in\beta(t)\) iff \(w\in\bar\beta(t)\); in particular
    \(\Phi(\bar\beta(t))=\beta(t)\), and \(\Bags(\Phi(w))=U_w\), where
    \(U_w=\{s\in N(T):w\in\bar\beta(s)\}\) is the occurrence set of \(w\)
    from Lemma~\ref{lem:decode-occ}.
\end{enumerate}
\end{lemma}

\begin{proof}
Write \(\rho(t)=(H_t,R_t)\) for the letters of the encoding.  By construction,
\(V(H_t)=c(\beta(t))\) and \(R_t=c(\adh(t))\); for \(i,j\in V(H_t)\), we have
\(ij\in E(H_t)\) iff the color-\(i\) and color-\(j\) vertices of \(\beta(t)\)
are adjacent in \(G\); and for \(Q\in\Pred\), \(i\in Q^{H_t}\) iff the
color-\(i\) vertex of \(\beta(t)\) lies in \(Q^G\).

\emph{The slot map.}  If \(i\in V(H_t)=c(\beta(t))\), then \(\beta(t)\)
contains a vertex of color \(i\), which is unique by bag-injectivity; so
\(\phi(t,i)\) is well defined, lies in \(\beta(t)\), and has color \(i\).

\emph{\(\Phi\) is well defined, and (i) holds.}  It suffices to check that
\(\phi\) is constant on the generators of \(\sim\).  Let \(x\) be a non-root
node and \(i\in R_x=c(\adh(x))\).  The color-\(i\) vertex \(u\) of \(\adh(x)\)
lies in both \(\beta(x)\) and \(\beta(p(x))\), and by bag-injectivity it is
the unique color-\(i\) vertex of each; hence \(\phi(x,i)=u=\phi(p(x),i)\).
Therefore \(\Phi\) is well defined, and
\(c(\Phi([v(t,i)]))=c(\phi(t,i))=i=\bar c([v(t,i)])\).

\emph{Surjectivity.}  Let \(v\in V(G)\).  By vertex coverage
\(\Bags(v)\ne\varnothing\); pick \(t\in\Bags(v)\).  Then
\(c(v)\in c(\beta(t))=V(H_t)\) and \(\phi(t,c(v))=v\), so
\(v=\Phi([v(t,c(v))])\).

\emph{Injectivity.}  Suppose \(\Phi([v(s,j)])=\Phi([v(t,i)])=v\).  By (i),
\(j=c(v)=i\).  Since \(\phi(s,i)=v\), we have \(v\in\beta(s)\), and likewise
\(v\in\beta(t)\); thus \(s,t\in\Bags(v)\).  By the running-intersection axiom
\(\Bags(v)\) is connected, so the unique \(s\)--\(t\) path in \(T\) stays in
\(\Bags(v)\).  For every edge \(\{x,p(x)\}\) on this path,
\(v\in\beta(x)\cap\beta(p(x))=\adh(x)\), hence
\(i=c(v)\in c(\adh(x))=R_x\), so the edge lies in \(\Gamma_i\).  By
Lemma~\ref{lem:decode-occ}, \(v(s,i)\sim v(t,i)\), i.e.\
\([v(s,j)]=[v(t,i)]\).

\emph{Bags: part (ii).}  If \(w\in\bar\beta(t)\), say \(w=[v(t,i)]\), then
\(\Phi(w)=\phi(t,i)\in\beta(t)\).  Conversely, suppose \(\Phi(w)\in\beta(t)\),
and put \(i\defeq c(\Phi(w))=\bar c(w)\).  Then
\(i\in c(\beta(t))=V(H_t)\), so the slot \(v(t,i)\) exists, and
\(\phi(t,i)=\Phi(w)\), because \(\Phi(w)\) is a vertex of color \(i\) in
\(\beta(t)\) and such a vertex is unique.  Thus \(\Phi([v(t,i)])=\Phi(w)\),
and injectivity gives \(w=[v(t,i)]\in\bar\beta(t)\).  Consequently
\(\Phi(\bar\beta(t))=\beta(t)\) and
\(\Bags(\Phi(w))=\{t:\Phi(w)\in\beta(t)\}=\{t:w\in\bar\beta(t)\}=U_w\).

\emph{Adjacency.}  Suppose \(w\) and \(w'\) are adjacent in \(\bar G\).  By
the definition of decoded adjacency there are a node \(u\) and slots
\(v(u,i)\in w\) and \(v(u,j)\in w'\) with \(ij\in E(H_u)\).  Then
\(\Phi(w)=\phi(u,i)\) and \(\Phi(w')=\phi(u,j)\), and \(ij\in E(H_u)\) says
precisely that \(\phi(u,i)\) and \(\phi(u,j)\) are adjacent in \(G\).
Conversely, suppose \(\Phi(w)\Phi(w')\in E(G)\).  By the edge-coverage axiom
there is a node \(u\) with \(\Phi(w),\Phi(w')\in\beta(u)\).  By (ii),
\(w=[v(u,i)]\) and \(w'=[v(u,j)]\) with \(i=\bar c(w)\) and \(j=\bar c(w')\);
then \(\phi(u,i)=\Phi(w)\) and \(\phi(u,j)=\Phi(w')\) are adjacent in \(G\),
whence \(ij\in E(H_u)\), and \(w\), \(w'\) are adjacent in \(\bar G\) with
witness \(u\).

\emph{Predicates.}  Let \(Q\in\Pred\) and \(w=[v(t,i)]\).  By the definition
of the decoded predicates, which are well defined by
Lemma~\ref{lem:decode-wd}, \(Q\) holds of \(w\) in \(\bar G\) iff
\(i\in Q^{H_t}\), which holds iff \(\phi(t,i)=\Phi(w)\in Q^G\).

Hence \(\Phi\) is a bijection that preserves and reflects adjacency and every
unary predicate, i.e.\ an isomorphism of \(\tau_{\Pred}\)-graphs, and it
satisfies (i) and (ii).
\end{proof}
...
node \(t\in\Bags(u)\cap\Bags(v)\) such that the color-renamed node letter \(H_t\)
contains the edge \(c(u)c(v)\).
\end{lemma}

\begin{proof}
If \(uv\in E(G)\), then some bag \(\beta(t)\) contains both endpoints.  Hence
\(t\in\Bags(u)\cap\Bags(v)\).  By Lemma~\ref{lem:decode-bag}, the letter \(H_t\)
is the graph \(G[\beta(t)]\) after renaming each vertex to its color, so \(H_t\)
contains the edge \(c(u)c(v)\).

Conversely, if such a node \(t\) exists, Lemma~\ref{lem:decode-bag} maps the edge
\(c(u)c(v)\) of the letter \(H_t\) to an edge of \(G[\beta(t)]\) between the unique
vertices of colors \(c(u)\) and \(c(v)\) in \(\beta(t)\).  Those unique vertices
are exactly \(u\) and \(v\), because \(t\in\Bags(u)\cap\Bags(v)\).
\end{proof}
...
\begin{proof}
The conjunct \(\varphi_{\legal}\) gives legality.  In a legal tree, the predicate
\(A_i(x)\) says that the decoded bag \(\beta(x)\) contains the color-\(i\) vertex,
by Lemma~\ref{lem:decode-bag}; for a non-root node \(x\), the predicate \(R_i(x)\)
says that the color-\(i\) vertex lies in the adhesion
\(\beta(x)\cap\beta(p(x))\), by Lemma~\ref{lem:decode-adh}.  With these
identifications, the remaining conjuncts are a literal \MSO{} translation of the
five conditions in Lemma~\ref{lem:defining-pairs}: connectedness of \(U\);
existence of color \(i\) in every bag of \(U\); existence of color \(i\) in each
non-top adhesion; absence of color \(i\) in the top adhesion; and absence of color
\(i\) in every dangling child adhesion.  Therefore, by
Lemma~\ref{lem:defining-pairs}, the formula holds exactly for defining pairs.
\end{proof}
...
The recognition and atomic lemmas above are stated for a legal tree and its
decoding.  Through the canonical isomorphism of Lemma~\ref{lem:decode-enc},
they transfer verbatim to the situation in which the tree is the encoding of a
given triple; this is the form in which they are used in the
translation-correctness proof below.

\begin{corollary}[Recognition over an encoding]\label{cor:recognition-enc}
Let \((T,\beta)\) be a binary tree-decomposition of a \(\tau_{\Pred}\)-graph
\(G\) of width at most \(\omega\), let \(c:V(G)\to C_\omega\) be injective on
every bag, and let \((T,\rho)\) be the encoding of \((G,c,(T,\beta))\).  Then,
with defining pairs and defining tuples taken with respect to
\((G,c,(T,\beta))\):
\begin{enumerate}[label=\textup{(\arabic*)}]
  \item \((T,\rho)\models\varphi_{\vtx,i}(U)\) iff \((U,i)\) is the defining
    pair of a vertex of \(G\);
  \item \((T,\rho)\models\varphi_{\vtx}(\tuple U)\) iff
    \(\tuple U=\tuple U^v\) for some vertex \(v\) of \(G\);
  \item \((T,\rho)\models\varphi_{\set}(\tuple U)\) iff
    \(\tuple U=\tuple U^S\) for some vertex set \(S\subseteq V(G)\);
  \item for all vertices \(u,v\in V(G)\) and every vertex set
    \(S\subseteq V(G)\):
    \((T,\rho)\models\varphi_{\adj}(\tuple U^u,\tuple U^v)\) iff
    \(uv\in E(G)\);
    \((T,\rho)\models\varphi_Q(\tuple U^v)\) iff \(v\in Q^G\), for each
    \(Q\in\Pred\);
    \((T,\rho)\models\varphi_{\eq}(\tuple U^u,\tuple U^v)\) iff \(u=v\); and
    \((T,\rho)\models\varphi_{\cont}(\tuple U^v,\tuple U^S)\) iff \(v\in S\).
\end{enumerate}
\end{corollary}

\begin{proof}
The encoding is legal by Lemma~\ref{lem:legal-iff-enc}, so
Lemmas~\ref{lem:vtx-i}, \ref{lem:vtx}, \ref{lem:set} and
\ref{lem:adj}--\ref{lem:cont} apply to \((T,\rho)\) with respect to its
decoding \((\bar G,\bar c,(T,\bar\beta))\).  Let \(\Phi:\bar G\cong G\) be the
isomorphism of Lemma~\ref{lem:decode-enc}.  By parts (i) and (ii) of that
lemma, \(\Phi\) matches colors and occurrence sets: for every
\(w\in V(\bar G)\), the defining pair of \(w\) with respect to the decoding
equals the defining pair of \(\Phi(w)\) with respect to \((G,c,(T,\beta))\).
Consequently \(\tuple U^w=\tuple U^{\Phi(w)}\) and, taking coordinatewise
unions over the vertices of a set,
\(\tuple U^{\bar S}=\tuple U^{\Phi(\bar S)}\) for every
\(\bar S\subseteq V(\bar G)\).  Since \(\Phi\) is a bijection onto \(V(G)\),
the defining pairs and defining tuples arising from the decoding are exactly
those arising from \((G,c,(T,\beta))\).  This proves (1)--(3).

For (4), by the matching above,
\(\tuple U^u=\tuple U^{\Phi^{-1}(u)}\), \(\tuple U^v=\tuple U^{\Phi^{-1}(v)}\)
and \(\tuple U^S=\tuple U^{\Phi^{-1}(S)}\); that is, the given tuples are the
defining tuples, in the decoding, of \(\Phi^{-1}(u)\), \(\Phi^{-1}(v)\) and
\(\Phi^{-1}(S)\).  Lemmas~\ref{lem:adj}, \ref{lem:pred}, \ref{lem:eq} and
\ref{lem:cont} then give the stated equivalences, because the
\(\tau_{\Pred}\)-isomorphism \(\Phi\) preserves and reflects adjacency, each
unary predicate \(Q\in\Pred\), equality, and membership: for instance,
\(\Phi^{-1}(u)\Phi^{-1}(v)\in E(\bar G)\) iff \(uv\in E(G)\), and
\(\Phi^{-1}(v)\in\Phi^{-1}(S)\) iff \(v\in S\).
\end{proof}
...
We adopt the standard fresh-variable convention: the auxiliary bound variables
introduced by the macros \(\varphi_{\legal}\), \(\varphi_{\vtx}\),
\(\varphi_{\set}\), \(\conn\), \(\topnode\), and \(\dang\) are renamed to be
pairwise distinct and disjoint from the tuple variables \(Z^x_i,Z^X_i\), so that
the substitutions defining \(\theta^\star\) are capture-free; formulas are read
modulo \(\alpha\)-equivalence.

\paragraph{Correctness of the translation.}
Fix the encoding \((T,\rho)\) of \((G,c,(T,\beta))\).  It is legal by
Lemma~\ref{lem:legal-iff-enc}, and by Corollary~\ref{cor:recognition-enc} the
recognition and atomic formulas evaluate on \((T,\rho)\) directly with respect
to \(G\): \(\varphi_{\vtx}\) and \(\varphi_{\set}\) recognize exactly the
defining tuples \(\tuple U^v\), \(\tuple U^S\) of vertices and vertex sets of
\(G\), and \(\varphi_{\adj}\), \(\varphi_Q\), \(\varphi_{\eq}\),
\(\varphi_{\cont}\) evaluate on them according to \(G\).  Given a
\(\tau_{\Pred}\)-MSO{} formula \(\theta\) and a
\emph{graph assignment} \(\alpha\) mapping each free individual variable of
\(\theta\) to a vertex and each free set variable to a vertex set, let the
\emph{tree assignment} \(\alpha^\star\) map each tuple \(\tuple Z_x\) to
\(\tuple U^{\alpha(x)}\) and each tuple \(\tuple Z_X\) to \(\tuple U^{\alpha(X)}\).

\begin{lemma}[Translation correctness]\label{lem:translation}
For every \(\tau_{\Pred}\)-MSO{} formula \(\theta\) and every graph assignment
\(\alpha\) to its free variables,
\[
  G\models\theta[\alpha]
  \quad\Longleftrightarrow\quad
  (T,\rho)\models\theta^\star[\alpha^\star].
\]
\end{lemma}

\begin{proof}
Induction on \(\theta\); throughout, \((T,\rho)\) is legal, being an encoding.

\emph{Atomic formulas.}  Under \(\alpha^\star\) every tuple variable is assigned a
defining tuple of \((G,c,(T,\beta))\), so
Corollary~\ref{cor:recognition-enc}(4) applies directly:
\(\varphi_{\eq}\) for \(x=y\), \(\varphi_{\adj}\) for \(\adj(x,y)\),
\(\varphi_Q\) for \(Q(x)\), and \(\varphi_{\cont}\) for \(x\in X\).  Each
gives \(G\models\theta[\alpha]\) iff \((T,\rho)\models\theta^\star[\alpha^\star]\).

\emph{Boolean connectives.}  The translation commutes with \(\neg,\wedge,\vee\)
and leaves \(\alpha^\star\) unchanged, so these cases are immediate from the
induction hypothesis.

\emph{Existential vertex quantifier} \(\theta=\exists x\,\psi\), with
\(\theta^\star=\exists Z^x_0\cdots\exists Z^x_\omega
(\varphi_{\vtx}(\tuple Z_x)\wedge\psi^\star)\).  If \(G\models\theta[\alpha]\),
choose a witness vertex \(a\); its defining tuple \(\tuple U^a\) satisfies
\(\varphi_{\vtx}\) by Corollary~\ref{cor:recognition-enc}(2), and the
induction hypothesis for
\(\psi\) and \(\alpha[x\mapsto a]\)---whose tree assignment is
\(\alpha^\star[\tuple Z_x\mapsto\tuple U^a]\)---gives
\((T,\rho)\models\psi^\star[\alpha^\star[\tuple Z_x\mapsto\tuple U^a]]\); thus
\(\tuple U^a\) witnesses \(\theta^\star[\alpha^\star]\).  Conversely, if
\((T,\rho)\models\theta^\star[\alpha^\star]\), a witnessing tuple \(W\) satisfies
\(\varphi_{\vtx}(W)\); Corollary~\ref{cor:recognition-enc}(2) gives
\(W=\tuple U^a\) for some vertex \(a\) of \(G\).  The induction hypothesis for \(\psi\) and
\(\alpha[x\mapsto a]\) then yields \(G\models\psi[\alpha[x\mapsto a]]\), so
\(G\models\theta[\alpha]\).

\emph{Existential set quantifier} \(\theta=\exists X\,\psi\) is identical, using
\(\varphi_{\set}\) and Corollary~\ref{cor:recognition-enc}(3): every vertex set
of \(G\) has a defining tuple satisfying \(\varphi_{\set}\), and every tuple
satisfying \(\varphi_{\set}\) is the defining tuple of a vertex set of \(G\).

\emph{Universal quantifiers} reduce to the existential cases through
\(\forall q\,\psi\equiv\neg\exists q\,\neg\psi\).
\end{proof}

Taking \(\theta=\varphi\) a sentence (so \(\alpha\) and \(\alpha^\star\) are empty)
yields, for every \(\tau_{\Pred}\)-MSO{} sentence \(\varphi\),
\[
  G\models\varphi
  \quad\Longleftrightarrow\quad
  (T,\rho)\models\varphi_{\mathrm{tree}},
\]
because \(\varphi_{\mathrm{tree}}=\varphi_{\legal}\wedge\varphi^\star\) and the
encoding is legal.

\section{Completing the proof of Courcelle's theorem}

\begin{theorem}[Effective \MSO{}--tree-automaton correspondence;
Thatcher--Wright, Doner]\label{thm:automata}
Let \(\Sigma\) be a finite alphabet.  From an \MSO{} sentence \(\Phi\) over
\(\tau_\Sigma=\{\child_1,\child_2\}\cup\{P_a:a\in\Sigma\}\) one can compute a
deterministic bottom-up finite tree automaton \(\Aut_\Phi\) such that, for every
\(\Sigma\)-tree, \(\Aut_\Phi\) accepts it iff it satisfies \(\Phi\).  Running
\(\Aut_\Phi\) takes time linear in the number of nodes.
\end{theorem}

To fit a ranked-tree automaton model, encode a \(\Sigma\)-tree as a full binary
ranked tree by padding each absent child with a fresh null symbol \(\bot\)
(internal letters of rank \(2\), \(\bot\) of rank \(0\)); the relations
\(\child_1,\child_2\) and the label predicates \(P_a\) are first-order definable in
this encoding, so the translation of \(\Phi\) is faithful.  Over ranked trees the
\MSO{}-definable and the regular tree languages coincide effectively
\cite{ThatcherWright1968,Doner1970,TATA}; determinising the resulting automaton by
the bottom-up subset construction yields the deterministic \(\Aut_\Phi\), whose run
is a single bottom-up pass.

\TODO{This theorem is currently cited externally (Thatcher--Wright, Doner).
Neither \texttt{mathlib} nor \texttt{cslib} provides a tree-automata API, so a
self-contained proof must be developed from scratch (finite tree automata,
determinisation/closure, and the \MSO{}-to-automaton compilation).  See the
companion file \texttt{tree\_automaton\_roadmap.tex} for the list of required
components.}
...
over the finite set \(\Sigma_\omega\).  By Lemma~\ref{lem:translation},
\[
  G\models\varphi
  \quad\Longleftrightarrow\quad
  (T,\rho)\models\varphi_{\mathrm{tree}}.
...
By Theorem~\ref{thm:automata} applied to the finite alphabet \(\Sigma_\omega\) and
the sentence \(\varphi_{\mathrm{tree}}\), one computes a deterministic bottom-up
finite tree automaton \(\Aut\), depending only on \(\omega\), \(\Pred\), and
\(\varphi\), that accepts exactly the \(\Sigma_\omega\)-trees satisfying
\(\varphi_{\mathrm{tree}}\).  Running \(\Aut\) on \((T,\rho)\) is a single
bottom-up pass, linear in \(|N(T)|\).  Accept iff \(\Aut\) accepts.  The displayed equivalence proves
...
\begin{remark}
The only external ingredient in the last paragraph is Theorem~\ref{thm:automata},
the effective \MSO{}--tree-automaton correspondence.  All graph-specific work is
contained in the encoding, decoding, and interpretation lemmas proved above.
\end{remark}
...
\newblock \emph{Journal of Computer and System Sciences}, 4(5):406--451, 1970.

\bibitem{TATA}
Hubert Comon, Max Dauchet, R\'emi Gilleron, Florent Jacquemard, Denis Lugiez,
Christof L\"oding, Sophie Tison, and Marc Tommasi.
\newblock \emph{Tree Automata Techniques and Applications}.
\newblock 2007. \url{http://tata.gforge.inria.fr/}.
