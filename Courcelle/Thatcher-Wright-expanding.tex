...
function by its graph.  Conversely, for a nondeterministic automaton
\(\NAut=(R,\rho)\), take the deterministic powerset automaton with state set
\(\mathcal P(R)\).  The transition for \(f\in\SigmaSym_n\) maps
\((S_1,\ldots,S_n)\) to the set of all states reachable by an \(f\)-transition
from \(S_1,\ldots,S_n\).  If \(R_F\) is the nondeterministic accepting set, the
deterministic accepting states are
\[
  \{S\subseteq R:S\cap R_F\neq\emptyset\}.
\]
An induction on terms proves that the powerset run returns exactly the set of
nondeterministic states reachable on the same term.
\end{proof}
...
\begin{proof}[Construction]
Use deterministic automata.  Complement is obtained by replacing \(A_F\) by
\(A\setminus A_F\).  Intersection is obtained by the product automaton with
state set \(A\times B\) and accepting states \(A_F\times B_F\).  Union follows
from complement and intersection, or directly by the product automaton with
accepting set \((A_F\times B)\cup(A\times B_F)\).
\end{proof}
