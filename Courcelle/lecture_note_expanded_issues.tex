...
slightly generalized from the vocabulary \(\{\operatorname{edge}\}\) to a fixed
...
\(\{\operatorname{edge}\}\), the unary colours can instead be encoded by fixed
...
structures \((V,\operatorname{edge})\).  It should first be upgraded to a proof
...
\begin{itemize}[leftmargin=*]
  ...
  \item which labelled pairs satisfy \(\operatorname{Adj}_I\);
...
\paragraph{Status (2026-06).} Deferred pending a decision with collaborators;
the main note is left unchanged for now.  The planned route is the coloured
incidence structure above, adding unary predicates \(\mathsf{Vert}\) and
\(\mathsf{EdgeObj}\).  A brief check of the tree-language translation shows no
fundamental obstruction: the alphabet \(\Sigma_\omega\) stays finite (each letter
additionally tags every present colour as \(\mathsf{Vert}\) or
\(\mathsf{EdgeObj}\)); the atomic translations \(\mathsf{Vert}(x)^\dagger\) and
\(\mathsf{EdgeObj}(x)^\dagger\) only read the tag of the representative's top
letter.  The one nontrivial addition is that \(\varphi_{\mathrm{legal}}\) must
gain a parent--child agreement condition on the unary tags of shared colours,
together with the corresponding coherence lemma, so that the decoded unary
predicates are well-defined on equivalence classes.
...
\paragraph{Status (2026-06).} Partially addressed in
\texttt{lecture\_note\_expanded.tex}.  A remark after Theorem~1 now records the
normalized decomposition with \(|N(T)|=O(|V(G)|)\) and the honest bound
\(f(\omega,|\varphi|)\,(|V(G)|+|N(T)|)\), and separates the \(\Omega(|\varphi|)\)
cost of reading and compiling the sentence.  The statement of Theorem~1 itself is
left unchanged, pending a decision on whether the intended logical parameter is
\(|\varphi|\) or \(\operatorname{qr}(\varphi)\).
