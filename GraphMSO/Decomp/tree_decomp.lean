...
def HasWidth (D : TreeDecomposition G) (ω : ℕ) : Prop :=
  ∀ t : D.Node, (D.bag t).Finite ∧ (D.bag t).ncard ≤ ω + 1
...
noncomputable def rootDepth (T : RootedTreeDecomposition G) (t : T.decomp.Node) : ℕ :=
  T.decomp.T.dist T.root t

/-- The chosen root path realizes the graph distance from the root. -/
theorem rootPath_length_eq_rootDepth (T : RootedTreeDecomposition G) (t : T.decomp.Node) :
    (T.rootPath t).length = T.rootDepth t := by
  obtain ⟨p, hp, hlen⟩ := T.decomp.T_istree.isConnected.exists_path_of_dist T.root t
  have hpath :
      (⟨p, hp⟩ : T.decomp.T.Path T.root t) =
        ⟨T.rootPath t, T.rootPath_isPath t⟩ :=
    T.decomp.T_istree.IsAcyclic.path_unique _ _
...
theorem rootDepth_ne_of_adj (T : RootedTreeDecomposition G) {t u : T.decomp.Node}
    (h : T.decomp.T.Adj t u) :
    T.rootDepth t ≠ T.rootDepth u := by
  simpa [rootDepth] using T.decomp.T_istree.dist_ne_of_adj T.root h

/-- Along an edge, root depths differ by exactly one. -/
theorem rootDepth_eq_add_one_or_eq_add_one (T : RootedTreeDecomposition G)
    {t u : T.decomp.Node} (h : T.decomp.T.Adj t u) :
    T.rootDepth t = T.rootDepth u + 1 ∨ T.rootDepth u = T.rootDepth t + 1 := by
  simpa [rootDepth] using T.decomp.T_istree.dist_eq_dist_add_one_of_adj T.root h

/-- A node has at most one adjacent node of smaller root depth. -/
theorem eq_of_adj_of_rootDepth_lt {T : RootedTreeDecomposition G}
    {x parent₁ parent₂ : T.decomp.Node}
    (h₁ : T.decomp.T.Adj parent₁ x) (h₂ : T.decomp.T.Adj parent₂ x)
  ...
  have hpath₁ : T.rootPath x = (T.rootPath parent₁).concat h₁ := by
    let p : T.decomp.T.Walk T.root x := (T.rootPath parent₁).concat h₁
    ...
    have hpath :
        (⟨T.rootPath x, T.rootPath_isPath x⟩ : T.decomp.T.Path T.root x) =
          ⟨p, hp_path⟩ :=
      T.decomp.T_istree.IsAcyclic.path_unique _ _
...
theorem rootDepth_lt_of_cons_isPath_of_rootDepth_eq_add_one
    {T : RootedTreeDecomposition G} {parent child y : T.decomp.Node}
    (hpc : T.decomp.T.Adj parent child)
    (hdepth : T.rootDepth child = T.rootDepth parent + 1)
    (p : T.decomp.T.Walk child y) (hp : (SimpleGraph.Walk.cons hpc p).IsPath) :
...
theorem rootDepth_eq_parent_add_one (T : RootedTreeDecomposition G)
  ...
  have hadj : T.decomp.T.Adj (T.parent t) t := T.parent_adj hroot
  rcases T.rootDepth_eq_add_one_or_eq_add_one hadj with hbad | hgood
  · have hpos : 0 < T.rootDepth t := by
      simpa [rootDepth] using T.decomp.T_istree.isConnected.pos_dist_of_ne hroot.symm
    let q : T.decomp.T.Walk T.root (T.parent t) := (T.rootPath t).dropLast
    ...
    have hdist_le : T.rootDepth (T.parent t) ≤ T.rootDepth t - 1 := by
      simpa [rootDepth, q, parent, hq_len] using T.decomp.T.dist_le q
    omega
  · exact hgood

theorem IsChild.rootDepth_eq_add_one {T : RootedTreeDecomposition G}
    {parent child : T.decomp.Node} (h : T.IsChild parent child) :
...
theorem IsAncestor.rootDepth_le {T : RootedTreeDecomposition G}
    {ancestor node : T.decomp.Node} (h : T.IsAncestor ancestor node) :
...
theorem IsAncestor.comparable_of_common_descendant {T : RootedTreeDecomposition G}
    {a b x : T.decomp.Node} (ha : T.IsAncestor a x) (hb : T.IsAncestor b x) :
    T.IsAncestor a b ∨ T.IsAncestor b a := by
  let r : T.decomp.Node -> T.decomp.Node -> Prop := fun parent child => T.IsChild parent child
...
theorem IsAncestor.exists_isPath_and_forall_rootDepth_le
    {T : RootedTreeDecomposition G} {ancestor node : T.decomp.Node}
    (h : T.IsAncestor ancestor node) :
    ∃ p : T.decomp.T.Walk ancestor node,
      p.IsPath ∧ ∀ z : T.decomp.Node, z ∈ p.support -> T.rootDepth z ≤ T.rootDepth node := by
  induction h with
  | refl =>
