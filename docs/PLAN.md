...
- 목표: Hamiltonian cycle MSO2 sentence를 mathlib의 `Walk.IsHamiltonianCycle`와 연결한다. 구현:
  `GraphMSO/Examples.lean`의 `Examples.hamiltonian`, edge-set 기반 중간 술어
  `SimpleGraph.HasHamiltonianCycleByEdges`와 `Examples.satisfiesAt_hamiltonian_iff`,
  그리고 다리 보조정리 `SimpleGraph.hasHamiltonianCycleByEdges_iff` (2-regular spanning-connected
  edge set ↔ `∃ a p, p.IsHamiltonianCycle`, `[Fintype][DecidableEq][Nonempty]` 하에서; mathlib
  `IsCycles` 인프라로 정·역방향 모두 증명)와 typeclass-gated 따름정리
  `Examples.satisfiesAt_hamiltonian_iff_isHamiltonianCycle`. mathlib `IsHamiltonian`은 singleton
  관례가 있어 `∃ a p, IsHamiltonianCycle`를 직접 목표로 삼는다.
...
- mathlib에 이미 있는 정의와 프로젝트-local 정의를 구분하고 이름을 정리한다.
  (clique/independent/vertex cover/bipartite/perfect matching/connectivity/coloring/Hamiltonian은
  mathlib 정의 `IsClique`/`IsIndepSet`/`IsVertexCover`/`IsBipartite`/`Subgraph.IsPerfectMatching`/
  `Preconnected`/`Colorable`/`Walk.IsHamiltonianCycle`로 연결 완료. dominating은 mathlib에 대응
  정의가 없어 local `IsDominating`를 유지한다. minor는 mathlib에 graph minor 관계가 없어
  local `IsMinorModelBySets`/`HasK3MinorBySets`를 유지한다. 즉 mathlib에 대응이 있는 술어의
  연계는 사실상 마무리되었다.)
