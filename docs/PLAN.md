...
`Courcelle/lecture_note.pdf`의 주 증명 경로는 decomposition 위에서 직접 DP를 먼저
정의하지 않는다. 대신 width `ω`의 binary tree decomposition과 bag-injective
`(ω + 1)`-coloring을 finite alphabet `Σ_ω` 위 labeled tree로 바꾼다. 여기서
`Σ_ω`의 문자는 `Fin (ω + 1)`의 부분집합 위 rooted graph `(H, R)`로 볼 수 있다.
...
- automaton evaluator가 tree 크기에 선형으로 실행된다는 statement를 준비한다. 복잡도 증명은
  약한 형태에서 시작해도 된다.
- MSO tree sentence마다 해당 tree automaton이 존재한다는 regularity theorem을 interface로 둔다.
  초기에는 theorem statement 또는 typeclass/axiom-free placeholder로 경계를 세우고, proof는 별도 장기 과제로 둔다.
...
1. `Assignment.AgreeOnFree` 기반 assignment-independence 정리를 끝낸다.
2. tree decomposition API를 binary/rooted/adhesion/cone/`BAGS(v)` 방향으로 보강한다.
3. `(ω + 1)`-coloring과 `Σ_ω` alphabet, legal `Σ`-tree encoding을 정의한다.
4. graph MSO1 over `{edge}`에서 tree MSO로 가는 translation correctness를 세운다.
5. finite tree automata interface를 추가하고, decomposition-given Courcelle statement를 먼저 세운다.
6. MSO2는 우선 incidence theorem-level 번역으로 연결한다.
