...
- `KRootedGraph`를 rooted **`τ_P`-structure**로 일반화한다(현재는 평범한 그래프만
  담음). letter가 인접 외에 색별 unary predicate tag(`P(x)`)를 운반해야 한다.
...
- 현재 MSO2 syntax와의 연결: ① `P = ∅`인 edge-sort-free **MSO1 over `{adj}`**
  ...
  채택). `GraphMSO/incidence.lean`의 uncoloured `IncidenceGraph`(생성자
  `fromV`/`fromEdge`)를 생성자 유도 `Vert`/`EObj`를 노출하는 coloured
  `τ_I`-structure로 승격하는 것이 선행 작업이다.
