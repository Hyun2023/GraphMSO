...
- **`τ_P`-structure 일반화 (완료).** 인코딩 측을 `τ_P`-structure 위로 올렸다.
  - 코어 `KRootedPGraph P`(= `SimpleGraph` + `pred : P → V → Prop`)를
    `GraphMSO/Decomp/KRootedPGraph.lean`에 정의. `Fintype`/`DecidableEq P`는
    구조체에 박지 않고 필요한 정리에서만 가정. incidence instance는 6단계 참조.
  - `sigmaTree`: `SigmaLetter P ω`에 `tag : P → verts → Prop`, `Compatible`에
    공유 색의 tag 일치 조건 추가. `toKRootedGraph`는 `pred := A.tag` 산출.
  - `rootedGraph`: `KRootedGraph (P) (k) extends KRootedPGraph P`,
    `LabelCompatibleOnRoots`에 predicate 일치, `GluingData`에 `left_pred`/`right_pred`
    상속 절 추가. 기존 gluing 보조정리는 pred에 비의존이라 재증명 0.
  - `nodeConeGraph`: node/cone graph가 `vpred : P → V → Prop`를 파라미터로 받아
    `pred := fun p x => vpred p x.1`로 bag/cone에 제한해 운반. `TreeDecomposition`은
    `SimpleGraph` 기반 그대로(`tree_decomp`/`bagColoring`/`nice`/`bags` 무수정).
...
- `Σ_ω` 문자 타입 `SigmaLetter P ω`(rooted `τ_P`-structure: `verts`/`G`/`R` + 색별
  `tag : P → verts → Prop`)와 `SigmaTree P ω`(`SimpleGraph.IsTree` 기반 rooted tree).
- 각 decomposition node의 node graph/cone graph를 `τ_P`-structure로 정의
  (`nodeConeGraph.lean`의 `nodeGraph`/`coneGraph`, `vpred` 운반).
- gluing 기계: `KRootedGraph`의 `Gluable`/`LabelCompatibleOnRoots`/`GluingData`/
  `IsGluing` (`rootedGraph.lean`), predicate 일치·상속 포함.
- `Σ`-tree legality: `SigmaLetter.Compatible`(공유 색의 인접+unary 일치) +
  `SigmaTree.IsLegal`(+ root `R = ∅`), `IsLegal.compatible_of_isChild` 등.
- letter→rooted graph 변환 `SigmaLetter.toKRootedGraph`(색을 라벨로).
...
- **인코딩 함수**: bounded-width colored decomposition `(T, β, c)`와 predicate family
  `vpred`의 각 노드 node graph를 색으로 rename해 `SigmaLetter`를 만들고 `SigmaTree`로
  조립한다.
- **cone-gluing 보조정리**: cone graph at `t` = node graph ⊕ 자식 cone graph들
  (`GluingData`/`IsGluing`로 서술), bag 라벨 컨벤션 포함.
- **디코딩 함수**: legal `Σ`-tree를 gluing으로 풀어 `(G, c, T)`를 복원한다.
- encode∘decode / decode∘encode equivalence(강의노트 Lemma 7-9 대응)를 증명한다.
...
5. tree decomposition API와 `Σ_ω` alphabet/legality/gluing을 갖추고, 인코딩 측을
   `τ_P`-structure로 일반화한다(`KRootedPGraph` 코어, letter `tag`, node/cone `vpred`,
   `colouredIncidence` instance).

다음 작업:

1. `Σ`-tree 인코딩 함수(decomposition + coloring + `vpred` → `SigmaTree`)와 cone-gluing
   보조정리, 디코딩 함수, encode/decode correctness를 세운다(5단계 남은 작업).
2. `τ_P` 위 MSO(우선 `P = ∅`인 graph MSO1 over `{adj}`)에서 tree MSO로 가는
   translation correctness를 세운다(6단계).
3. MSO2는 `colouredIncidence` instance를 거쳐 incidence 환원으로 연결한다(6단계).
4. finite tree automata interface를 추가하고, decomposition-given Courcelle statement를 먼저 세운다(7단계).
