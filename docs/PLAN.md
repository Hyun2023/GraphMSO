...
second-order logic을 형식화하는 것이다. 현재 범위는 정점 집합을 2차 변수로
다루는 MSO1이다. 즉 1차 변수는 정점에 할당되고, 2차 변수는 mathlib의
`Set V` 꼴 정점 집합에 할당된다. 간선 집합을 2차 변수로 다루는 MSO2는
당장 목표가 아니며, 필요해질 때 별도 확장으로 설계한다.

프로젝트는 이제 mathlib에 의존한다. 기본 그래프 표현은 여전히 작은 관계 기반
`Graph V`로 유지하되, 정점 집합은 `Set V`를 사용하고, mathlib `SimpleGraph`와
...
- `GraphMSO.Basic`
  ...
  - `Graph.Irreflexive`와 `Graph.Symmetric`은 mathlib/core 관계 술어를 사용한다.
  - `VSet V := Set V`로 정점 집합을 표현한다.
...
- MSO 의미론은 작은 관계 기반 `Graph V` 위에 둔다.
...
목표는 MSO1의 최소 커널을 안정화하는 것이다. 상당 부분은 이미 구현되어 있고,
남은 작업은 theorem API를 더 쓰기 좋게 만드는 쪽이다.
...
- 표현 가능성 예제를 모은다.
  ...
  - minor 관련 인코딩은 MSO1에서 직접 가능한 범위와 MSO2가 필요한 범위를 구분한 뒤
    진행한다.
